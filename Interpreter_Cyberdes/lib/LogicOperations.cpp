...
void shlOperation(std::vector<std::variant<unsigned int, int, size_t, std::string>>& stack) {
    if (stack.size() < 2) {
        throw std::runtime_error("Not enough elements in stack for shl operation");
    }

    auto var_b = stack.back(); stack.pop_back();
    auto var_a = stack.back(); stack.pop_back();

    // Check if both a and b are ints for the operation
    if (std::holds_alternative<int>(var_a) && std::holds_alternative<int>(var_b)) {
        int a = std::get<int>(var_a);
        int b = std::get<int>(var_b);
        ...
        throw std::runtime_error("shl operation requires numeric operands");
    }
}

void shrOperation(std::vector<std::variant<unsigned int, int, size_t, std::string>>& stack) {
    if (stack.size() < 2) {
        throw std::runtime_error("Not enough elements in stack for shr operation");
    }
    ...
    if (std::holds_alternative<int>(var_a) && std::holds_alternative<int>(var_b)) {
        ...
        throw std::runtime_error("shr operation requires numeric operands");
    }
}